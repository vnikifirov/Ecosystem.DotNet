...
namespace corelib
{
    ...
    public class SunEnv : GuiEnv
    {
        ...
        public SunEnv(DataParamTable par) :
        ...
        {
            ViewMultiTupleForm(tupels, false).ShowDialog();
        }

        public Form ViewMultiTupleForm(IMultiDataProvider tupels, bool extra)
        {
            DataTupleVisualizer dv3 = new DataTupleVisualizer(this);
            dv3.SetDataProvider(tupels);
            if (extra)
            {
                dv3.RegisterAction(new DTVSunEnvAdd());
                dv3.RegisterAction(new DTVSunEnvExport());
                dv3.RegisterAction(new DTVSunEnvRecover());
                dv3.RegisterAction(new DTVSunEnvOpendb());
#if !DOTNET_V11                
                //dv3.RegisterAction(new DTVPluginDTVSunEnvTotalCompute());
#endif
                
                dv3.RegisterAction(new DTVSunEnvSnapshot());
            }
            else
            {
                dv3.RegisterAction(new DTVSunEnvSnapshot());
        ...
        // Далее идут процедуры, специфичные по восстановлению расходов
        ...
        public IMultiDataTuple RecoveryDataAllCart(IMultiDataProvider p, DateTime dt, DateTime prev, IMultiDataTuple coeff)
        {
            IMultiDataTuple all = GetAllData(p, dt);
            IMultiDataTuple allPrev = GetAllData(p, prev);
            IMultiDataTuple rec = CombineRecoveredIntoCartogram(RecoveryData(p, dt, coeff), all, allPrev);
            return DataTuple.Combine(null, all[0], rec[0]);
        }

        public IMultiDataTuple[] SimpleRecovery(IMultiDataProvider p, DateTime dateTo, DateTime dateForCoeff)
        {
            IMultiDataTuple coeff = ComputeAdaptationCoefficient(p, dateForCoeff);
            IMultiDataTuple prev = GetAllData(p, dateForCoeff);

            IMultiDataTuple recover = RecoveryDataAllCart(p, dateTo, dateForCoeff, coeff);
            return new IMultiDataTuple[] { recover, prev };
        }
        ...
        public IMultiDataTuple[] SimpleRecoveryWithCoeffRecoveryAndNewGamma(IMultiDataProvider p, DateTime dateTo, DateTime dateForCoeff)
        {
            IMultiDataTuple coeffNew = ComputeAdaptationCoefficient(p, dateTo);
            IMultiDataTuple coeff = ComputeAdaptationCoefficient(p, dateForCoeff);
            IMultiDataTuple prev = GetAllData(p, dateForCoeff);
            IMultiDataTuple curr = GetAllData(p, dateTo);

            IMultiDataTuple coeffM = GetAlgorithm("correctUnknownAzotCoeff").CallIntelli(
                zagrConvToInt.CallIntelli(prev), coeff);
            IMultiDataTuple coeffNewM = GetAlgorithm("correctUnknownAzotCoeff").CallIntelli(
                zagrConvToInt.CallIntelli(curr), coeffNew);

            IMultiDataTuple recover = RecoveryDataAllCart(p, dateTo, dateForCoeff,
                new MultiDataTuple("tmp", dateTo,
                    coeffM["bet1"],
                    coeff["kpd"],
                    coeffNew["gamma"]));

            IDataTuple coeffcart = CombineCoeffsIntoCartogram(coeff, prev);
            IDataTuple coeffcartNew = CombineCoeffsIntoCartogram(coeffNew, curr);


            DataTuple rcur = DataTuple.Combine(null, recover[0], coeffcartNew);
            DataTuple rprev = DataTuple.Combine(null, prev[0], coeffcart);

            return new IMultiDataTuple[] { rcur, rprev };

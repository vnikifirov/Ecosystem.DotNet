...
namespace ServiceRecoveryEmulation
{
    ...
    public class RecoveryPlugin
    {
        ...
        public void WorkService()
        {
            ...
            /*
            if (File.Exists("C:\\tmp\\skala-azot.txt") == true)
                File.Delete("C:\\tmp\\skala-azot.txt");
            StreamWriter sw = new StreamWriter("C:\\tmp\\skala-azot.txt", false, Encoding.GetEncoding(1251));
            sw.WriteLine("skala_date;azot_date;delta_time;1/delta_time;");
            */
                ...
                */
                ...
                if (dt_az.Length > azot_count)//если поступила новая прописка
                {
                    ...
                    */
                    ...
                    {
                        //sw.WriteLine(dataSkala.GetTimeDate().ToString() + ";" + mdt_az.GetTimeDate().ToString() + ";;;");
                        for (int n = 0; n < 48; n++)
                        {
                            for (int m = 0; m < 48; m++)
                            {
                    ...
                    else
                    {
                        ...
                        /*
                        sw.WriteLine(dataSkala.GetTimeDate().ToString() + ";" + mdt_az.GetTimeDate().ToString() + ";" + dtime.ToString() + ";" + (1 / dtime).ToString() + ";");
                        if (dt_az.Length == 21)
                            sw.Close();
                        */
                        ...
                        for (int j = 0; j < 48; j++)
                        {
                            ...
                            if (fc.Fiber != -1)
                            {
                                ...
                                if (Math.Abs(Convert.ToDouble(bet1_prev_dar[fc.Fiber][fc.Pvk]) - Convert.ToDouble(bet1_dar[fc.Fiber][fc.Pvk])) > 0.3 * Math.Abs(Convert.ToDouble(bet1_prev_dar[fc.Fiber][fc.Pvk])))
                                    f = true;//скачок озотного коэффициента
                                if (dp_int_cart[i, j] == 0)
                                {

...
namespace ServiceEmulation
{
    ...
    public class RockDataPlugin
    {
        ...
        public RockDataPlugin(ServiceForm.TupEnv env)
        {
            string[] file_lines = File.ReadAllLines("dates.txt");
            string[] coords = new string[file_lines.Length];

            IFormatProvider ifp = new System.Globalization.CultureInfo("ru-RU");
            ...
            for (int i = 0; i < file_lines.Length; i++)
            {
                ...
                //begin_dates[i] = DateTime.ParseExact(
                begin_dates[i] = Convert.ToDateTime(line_parts[1],ifp);
                end_dates[i] = Convert.ToDateTime(line_parts[2],ifp);
            }
            ...
            while (true)
            {
                ...
                try
                {
                    //занесение зрк
                    string sday = null;
                    if (dataSkala_new.GetTimeDate().Day < 10)
                        sday = "0" + dataSkala_new.GetTimeDate().Day.ToString();
                    else
                        sday = dataSkala_new.GetTimeDate().Day.ToString();
                    string smonth = null;
                    if (dataSkala_new.GetTimeDate().Month < 10)
                        smonth = "0" + dataSkala_new.GetTimeDate().Month.ToString();
                    else
                        smonth = dataSkala_new.GetTimeDate().Month.ToString();
                    string syear = null;
                    if (dataSkala_new.GetTimeDate().Year < 10)
                        syear = "0" + dataSkala_new.GetTimeDate().Year.ToString();
                    else
                        syear = dataSkala_new.GetTimeDate().Year.ToString().Substring(2);
                    string zrk_path = "C:\\tmp\\Тест\\1_" + sday + smonth + syear + ".txt";
                    OvlInfoProvider zrk = new OvlInfoProvider(zrk_path);
                    IDataTuple tn = (IDataTuple)(zrk.GetData());
                    ITupleItem i_zrk = tn[0];

                    i_zrk = i_zrk.Rename(new TupleMetaData(i_zrk.Name, i_zrk.HumaneName, dataSkala_new.GetTimeDate(), dataSkala_new.GetStreamName()));
                    tn = new DataTuple(dataSkala_new.GetStreamName(), dataSkala_new.GetTimeDate(), i_zrk);
                    string[] names_zrk = new string[dataSkala_new.ItemsCount + tn.ItemsCount];
                    dataSkala_new.CopyNamesTo(names_zrk);
                    tn.CopyNamesTo(names_zrk, dataSkala_new.ItemsCount);
                    DataTuple comby_zrk = DataTuple.Combine(names_zrk, dataSkala_new, tn);

                    dataSkala_new = new DataTuple(comby_zrk, dataSkala_new.GetStreamName(), dataSkala_new.GetTimeDate());
                    //конец зрк
                }
                catch
                {
    ...
    public class AzotDataPlugin
    {
        ...
        public AzotDataPlugin(ServiceForm.TupEnv env)
        {
            while (true)
            {
                try

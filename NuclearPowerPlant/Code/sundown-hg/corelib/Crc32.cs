...
namespace corelib
{
    public class Crc32 : HashAlgorithm
    {
        ...
        public static void ComputeCrc32(byte[] buffer, int start, int length, ref uint lcrcValue)
        {
            lcrcValue ^= DefaultSeed;

            unchecked
            {
                while (--length >= 0)
                {
                    lcrcValue = CrcTable[(lcrcValue ^ buffer[start++]) & 0xFF] ^ (lcrcValue >> 8);
                }
            }
        ...
        {
            return ComputeCrc32(buffer, buffer.Length);
        }

        public static uint ComputeCrc32(byte[] buffer, int len)
        {
            uint val = 0;
            ComputeCrc32(buffer, 0, len, ref val);
            return val;
        }

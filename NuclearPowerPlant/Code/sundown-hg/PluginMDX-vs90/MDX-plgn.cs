...
namespace PluginMDX_vs90
{
    ...
    public class MDXplugin
    {
        protected readonly CoordsConverter _liner1884;
        protected readonly CoordsConverter _liner2448;

        MDXplugin(CoordsConverter liner1884, CoordsConverter liner2448)
        {
            _liner1884 = liner1884;
            _liner2448 = liner2448;
            
        }

        void InitZagr2448(string filename, String name, String humanhelp, out DataCartogramIndexedByte c)
        {
           byte[] tmp = new byte[4976];
           
           using (FileStream fs = File.OpenRead(_path + filename))
                fs.Read(tmp, 0, 4896);

           // int[] data = new int[2448];
           short[] data = new short[2448];
           //TYT 2488 A HE 2448!!!!!!!!!!!!!!!!!!!!
           for (int i = 0; i < 2448; i++)
           {
               data[i] = BitConverter.ToInt16(tmp, i * 2);
           }

           byte[] tmp2 = new byte[2448];
           for (int i = 0; i < 2448; i++)
           {
               tmp2[i] = System.Convert.ToByte(data[i]);
           }

            StreamDeserializer d = new StreamDeserializer(tmp2, 0);

            DataArrayByte array = new DataArrayByte(new TupleMetaData(name, humanhelp, _dateTime, TupleMetaData.StreamAuto),
                d, 2448, 0, 0);

            c = new DataCartogramIndexedByte(array.Info, ScaleIndex.Default, _liner2448, array);
        ...
        void ReadFlow_MDX(String file, String name, String humanhelp, out DataCartogramIndexedFloat c)
        {
            //извлечение расхода из SADA.MDX
          
            byte[] fullData = GetFileData(file);
            StreamDeserializer d = new StreamDeserializer(fullData, 0x1C8);
            DataArrayFloat array = new DataArrayFloat(new TupleMetaData(name, humanhelp, _dateTime, TupleMetaData.StreamAuto),
                d, 1884, 0, 0);
            ScaleIndex scale = new ScaleIndex(0, 0.01);

           
            
            c = new DataCartogramIndexedFloat(array.Info, ScaleIndex.Default, _liner1884, array);
           // return 0;
        }
        ...
        void InitRestartOfSkalaINTE_MDX(String filename)
        {
            byte[] fullData = GetFileData(filename);
            StreamDeserializer d = new StreamDeserializer(fullData, 0x35180);
            DataArrayFloat array = new DataArrayFloat(new TupleMetaData("energovir", "Энерговыработка ТВС реактора (МВт*сут)", _dateTime, TupleMetaData.StreamAuto),
                d, 1884, 0, 0);

            _energovir = new DataCartogramIndexedFloat(array.Info, ScaleIndex.Default, _liner1884, array);
        }

        void InitZAPK_MDX(String filename)
        {
            byte[] fullData = new byte[5555];// GetFileData(filename);
        ...
        void InitRestartOfSkalaNPHR_MDX(String filename)
        {
            byte[] fullData = GetFileData(filename);

            StreamDeserializer d = new StreamDeserializer(fullData, 0x1370);
            DataArrayFloat array = new DataArrayFloat(new TupleMetaData("fizras", "Мощности ТК по ОНФР-2", _dateTime, TupleMetaData.StreamAuto),
                            d, 1884, 0, 0);

            ScaleIndex scale = new ScaleIndex(0, 0.01);
            _fizras = new DataCartogramIndexedFloat(array.Info, scale, _liner1884, array);
        }
        ...
        protected void Init(string filepath, bool readDateFromPath)
        {
            ...
            {
                //MDX-format
                InitZagr2448("SANR.MDX", "zagr_rockmicro", "Загрузка, СКАЛА-Микро", out _zagr);
            }
            catch (FileNotFoundException)
            {              
        ...
        public MDXplugin(CoordsConverter liner1884, CoordsConverter liner2448, string filepath, bool readDateFromPath)
            : this (liner1884, liner2448)
        {
            Init(filepath, readDateFromPath);
           
        }

        public MDXplugin(CoordsConverter liner1884, CoordsConverter liner2448, string filepath)
            : this (liner1884, liner2448)
        {
            Init(filepath, false);
        }
        ...
        //rock_micro -> rock_mdx
        public const string defaultStreamName = "rock_mdx";

        //protected DataCartogramNativeByte _zagr;
        protected DataCartogramIndexedByte _zagr;
        protected DataCartogram _power;
        //protected DataCartogramNativeDouble _flow;
        protected DataCartogramIndexedFloat _flow;
    ...
    [AttributeDataComponent("Чтение среза состояния СКАЛА-Микро", SourceNames = MDXplugin.sourceNames, ComponentFileFilter = "Данные базы СКАЛА-МИКРО KATKR.INI|KATKR.INI", ComponentFileNameArgument = "rockMicroSingleDirectoryPath")]
    public class RockMicroSingleProviderMDX : MDXplugin, ISingleDataProvider
    {
        public static readonly string[] splitedSourceNames = sourceNames.Split(',');
        const string initFile = "KATKR.INI";

        public RockMicroSingleProviderMDX(IEnviromentEx enviromentObject, string rockMicroSingleDirectoryPath)
            : base(enviromentObject.GetSpecialConverter(CoordsConverter.SpecialFlag.Linear1884, new TupleMetaData()), 
            enviromentObject.GetSpecialConverter(CoordsConverter.SpecialFlag.WideLinear2448, new TupleMetaData()),
    ...
    [AttributeDataComponent("Чтение множества прописок базы СКАЛА-Микро", MultiTuple = false, SourceNames = MDXplugin.sourceNames)]
    public class RockMicroMultiProviderMDX : SingleSearchToMultiProvider
    {
        public RockMicroMultiProviderMDX(IEnviromentEx enviromentObject, string rockMicroMultiDirectoryPath)
            : base(enviromentObject, new DataComponents.DataComponent(typeof(RockMicroSingleProviderMDX)), rockMicroMultiDirectoryPath)
        {

        }
    }
    /*
    public class RockMicroAlgoMDX
    {

...
namespace bank_identification_code.Controllers
{
    ...
    public class PZNController : Controller
    {
        ...
        private readonly IRepository<PZNEntity> repository;
        private readonly IUnitOfWork unitOfWork;
        private readonly IMapper mapper;

        public PZNController(IRepository<PZNEntity> repository, IEncoder converter,  IUnitOfWork unitOfWork, IMapper mapper)
        {
        ...
        [HttpGet]
        public async Task<IEnumerable<PZNResource>> Index()
        {
            var entitis = await repository.GetAllAsync();
            var apiResources = mapper.Map<List<PZNEntity>, List<PZNResource>>(entitis);
        ...
        public async Task<IActionResult> GetById(string VKEY)
        {
            ...
            var apiResource = mapper.Map<PZNEntity, PZNResource>(entity);
            // ? Trim trailing spaces from char fields
            // ? url: https://community.dynamics.com/gp/b/gpdynland/archive/2017/06/10/asp-net-core-and-ef-core-with-dynamics-gp-trim-trailing-spaces-from-char-fields
            apiResource = TrimStrings.TrimProps<PZNResource>(apiResource);
